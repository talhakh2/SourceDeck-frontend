...
import { Star, ShoppingCart, Eye, Heart, Share2, CheckCircle, ExternalLink, TrendingUp } from 'lucide-react';
...
export const ProductCard = React.memo(function ProductCard({ product, onViewDetails, className }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    if (!user) {
      toast.error('Please login to add items to cart', { duration: 2000 });
      return;
    }
    if (user.role !== 'buyer') {
      toast.error('Only buyers can add items to cart', { duration: 2000 });
      return;
    }
    addItem(product);
  };
  ...
  const handleLike = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setIsLiked(!isLiked);
    toast.success(isLiked ? 'Removed from favorites' : 'Added to favorites', { duration: 1500 });
  };
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 shadow-sm hover:shadow-md transition-all duration-200 group ${className}`}>
        ...
        <div className="flex gap-2">
          ...
          {isPurchasedByUser ? (
            <Link
              ...
              className="flex-1 btn btn-success btn-sm flex items-center justify-center gap-2"
            >
